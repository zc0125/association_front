<template>
<!-- 修改活动信息 -->
	<el-form :model="activity" label-width="100px" size="small" class="main">
      ...
      <el-select v-model="activity.clubId" placeholder="请选择社团名称" disabled>
        <el-option
		...
		update: function() {
			...
			this.$axios.put('/api/activities/', this.activity).then(res => {
				...
				if (res.data.code == OK) {
					...
					this.$router.push({ name: 'ActivityListUser' });
				} else {
