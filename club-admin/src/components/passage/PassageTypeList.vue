<template>
  <div>
    ...
    <el-table :data="newsTypeList" stripe style="width: 100%" border>
      ...
      <el-table-column prop="type" label="文章类型" min-width="150">
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    // 添加类型
    addType: function () {
      let data = {"type":this.passageTypeName}
      this.$axios.post("/api/passageTypes",data).then((res)=>{
        if(res.data.code == OK){
    ...
    editTypeInfo: function () {
      ...
      this.$axios.put("api/passageTypes",data).then((res)=>{
        if(res.data.code == OK){
    ...
    deleteTypeById: function (id) {
      this.$axios.delete("/api/passageTypes/" + id).then((res) => {
        if (res.data.code == OK) {
