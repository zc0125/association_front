...
export default {
  ...
  methods: {
    ...
    // 添加类型
    addType: function () {
      let data = {"type":this.clubTypeName}
      this.$axios.post("/api/clubTypes",data).then((res)=>{
        if(res.data.code == OK){
    ...
    editTypeInfo: function () {
      ...
      this.$axios.put("/api/clubTypes",data).then((res)=>{
        if(res.data.code == OK){
    ...
    deleteTypeById: function (id) {
      this.$axios.delete("/api/clubTypes/" + id).then((res) => {
        if (res.data.code == OK) {
