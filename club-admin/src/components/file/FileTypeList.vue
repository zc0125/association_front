<template>
  <div>
    ...
    <el-table :data="fileTypeList" stripe style="width: 100%" border>
...
export default {
  ...
  methods: {
    ...
    // 添加类型
    addType: function () {
      let data = {"type":this.fileTypeName}
      this.$axios.post("/api/fileTypes",data).then((res)=>{
        if(res.data.code == OK){
    ...
    editTypeInfo: function () {
      ...
      this.$axios.put("/api/fileTypes",data).then((res)=>{
        if(res.data.code == OK){
    ...
    deleteTypeById: function (id) {
      this.$axios.delete("/api/fileTypes/" + id).then((res) => {
        if (res.data.code == OK) {
